...
@users = pagination
  ...
  def index
	@subscription = Subscription.find(params[:subscription_id])
	if params[:search]
	  @users =@subscription.users.where("first_name LIKE '#{params[:search]}%' or last_name LIKE '#{params[:search]}%' or phone_no LIKE '#{params[:search].to_s}%' or reg_no LIKE '#{params[:search].to_s}%'").paginate(:page => params[:page], :per_page => 15).order('created_at DESC')
...
request.body = "{\"from\":\"NACOSS ISEC\",\"to\":\"#{@user.phone_no}\",\"text\":\"Hi #{@user.first_name},Your NACOSS E-voting Reg.Number is #{@user.reg_no},Password is #{@generated.to_s},follow this link to vote bit.ly/1RT5K9x\"}"
...
@user.update(sent_thanks:data['messages'][0]['status']['groupName'])
  ...
  @users = @subscription.users.paginate(:page => params[:page], :per_page => 15).order('created_at DESC')
end
